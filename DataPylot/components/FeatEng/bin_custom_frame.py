...
class BinCustomFrame(ttk.Frame):
    def __init__(self, parent, manager):
        ...
        top_label = ttk.Label(self, text="Customize Bins by Grouping Values", font=("Arial", 14))
        top_label.grid(row=0, column=0, padx=5, pady=(20, 5), sticky="n")

        self.create_group_btn = ttk.Button(self, text="Create New Bin", command=self.on_create_group_btn)
        ...
        self.group_label = ttk.Label(self, text="Current Bin:", font=("Arial", 12))
        ...
        self.next_btn = ttk.Button(btn_frame, text="Next Bin", width=14, state="disabled", command=self.on_next_btn)
        self.next_btn.pack(pady=(0, 50))

        self.delete_group_btn = ttk.Button(btn_frame, text="Delete Bin", width=14, state="disabled", command=self.on_delete_btn)
    ...
    def update_nav(self):
        if self.current_group_idx >= 0:
            ...
            self.group_label.config(text=f"Current Bin: {name}")
            ...
            self.group_label.config(text=f"Current Bin:")
            self.next_btn.config(state="disabled")
            self.prev_btn.config(state="disabled")
            self.delete_group_btn.config(state="disabled")
            self.update_group_display()
