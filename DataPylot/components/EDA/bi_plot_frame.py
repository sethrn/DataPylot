...
class BiPlotFrame(GenerateCodeFrame):
    def __init__(self, parent, manager):
        ...
        xvals = []
        yvals = [] # also used for hue
        ytext = "Select Y Axis Feature:"
       
        if self.plot_type.startswith('Grouped'):
            xvals = list(self.current_df.columns)
            yvals = list(self.current_df.columns)
            ytext = "Select Hue:"
        elif self.plot_type in ['Violin Plot', 'Swarm Plot']:
            xvals = list(self.current_df.columns)
        ...
        self.xaxis_err.grid(row=current_row+2, column=0, padx=(250,5), pady=(25,5), sticky="w")
        ...
        colors = ["tab10", "viridis", "cividis", "pastel", "RdBu"] if self.plot_type != 'Scatter Plot' else ['red', 'blue', 'green', 'yellow', 'orange', 'purple', 'pink', 'brown', 'black', 'white']
    ...
    def generate_code(self):
        ...
        code, imports = BivariatePlotGenerator.generate(
            df=self.df_name,
            plot=self.plot_type,
            x=x,
            y=y,
