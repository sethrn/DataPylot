...
from components.EDA.multi_plot_frame import MultiPlotFrame

class MultivariateFrames(SequentialFrameManager):
    def __init__(self, parent):
        super().__init__(parent)

        self.add_frame(MultivariateStep1, manager=self)

        self.show_frame(0)

        self.next_btn.config(state="disabled")

class MultivariateStep1(ttk.Frame):
    def __init__(self, parent, manager):
        ...
        about_path = base_dir / "edu" / "EDA" / "about_multi.txt"
    ...
    def on_df_dropdown_selected(self, event):
        ...
        if isinstance(self.current_df, pd.DataFrame):
            self.plot_dropdown.config(
                state="readonly",
                values=['Heatmap', 'Pair Plot', 'Violin Plot', 'Swarm Plot', 'Scatter Plot', 'Line Plot']
            )
    ...
    def on_plot_dropdown_selected(self, event):
        ...
        if not prev_selection:
            ...
            self.manager.add_frame(MultiPlotFrame, manager=self.manager)
